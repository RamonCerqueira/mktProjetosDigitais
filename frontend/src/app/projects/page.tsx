...
export default function ProjectsPage() {
  ...
  return (
    <main className="mx-auto max-w-6xl px-6 py-16">
      ...
      <div className="grid gap-6 md:grid-cols-2 xl:grid-cols-3">{projects.map((project) => <div key={project.id} className="card space-y-4"><div className="flex items-start justify-between gap-3"><div><p className="text-sm text-emerald-300">{project.category}</p><h2 className="text-2xl font-semibold">{project.title}</h2></div>{project.ranking && <span className="rounded-full bg-emerald-500/15 px-3 py-1 text-sm text-emerald-300">#{project.ranking}</span>}</div><p className="text-slate-300">{project.description}</p><div className="grid gap-2 text-sm text-slate-400"><div className="flex justify-between"><span>{project.techStack}</span><span>{project.sellerCity}/{project.sellerState}</span></div><div className="flex justify-between"><span>MRR R$ {project.monthlyRevenue}</span><span>{project.sellerName}</span></div><div className="flex justify-between"><span>Score</span><span>{project.score ?? "-"}/100</span></div><div className="flex justify-between"><span>Preço sugerido</span><span>R$ {project.suggestedPrice ?? "-"}</span></div>{project.suspicious && <p className="text-amber-400">Anúncio sinalizado pela heurística antifraude.</p>}</div><div className="flex items-center justify-between gap-2"><strong className="text-2xl">R$ {project.price}</strong><div className="flex gap-2"><button className="btn-secondary">Negociar</button><button className="btn-primary" onClick={() => checkout(project.id)}>Pagar</button></div></div></div>)}</div>
    </main>
  );
}
