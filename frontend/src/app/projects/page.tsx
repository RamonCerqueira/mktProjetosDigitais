...
import { Project, ReverseGeocode } from "@/types";
...
export default function ProjectsPage() {
  ...
  return (
    <main className="mx-auto max-w-6xl px-6 py-16">
      ...
      <div className="grid gap-6 md:grid-cols-2 xl:grid-cols-3">{projects.map((project) => <div key={project.id} className="card space-y-4"><div><p className="text-sm text-emerald-300">{project.category}</p><h2 className="text-2xl font-semibold">{project.title}</h2></div><p className="text-slate-300">{project.description}</p><div className="flex justify-between text-sm text-slate-400"><span>{project.techStack}</span><span>{project.sellerCity}/{project.sellerState}</span></div><div className="flex justify-between text-sm text-slate-400"><span>MRR R$ {project.monthlyRevenue}</span><span>{project.sellerName}</span></div><div className="flex items-center justify-between"><strong className="text-2xl">R$ {project.price}</strong><button className="btn-secondary">Negociar</button></div></div>)}</div>
    </main>
  );
}
