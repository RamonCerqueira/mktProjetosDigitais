  ...
  return (
    <main className="mx-auto max-w-6xl space-y-8 px-6 py-16">
      ...
      <section className="grid gap-6 lg:grid-cols-[1fr_1.2fr]">
        ...
        <div className="card"><h2 className="mb-4 text-2xl font-semibold">Meus projetos</h2><div className="space-y-4">{dashboard.myProjects.map((project) => <div key={project.id} className="rounded-xl border border-slate-800 p-4"><div className="flex items-center justify-between"><h3 className="font-semibold">{project.title}</h3><span className="text-sm text-emerald-300">{project.status}</span></div><p className="mt-2 text-sm text-slate-300">{project.description}</p><div className="mt-3 grid gap-1 text-sm text-slate-400"><p>Score: {project.score ?? "-"}/100 · {project.qualification ?? "-"}</p><p>Nível do dev: {project.sellerLevel ?? "-"}</p></div></div>)}{dashboard.myProjects.length === 0 && <p className="text-slate-400">Nenhum projeto cadastrado ainda.</p>}</div></div>
      </section>
